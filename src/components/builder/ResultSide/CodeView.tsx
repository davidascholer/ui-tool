...
import { serializeToReact } from '@/utils/serialize/reactSerializer';
import { serializeToReactNative } from '@/utils/serialize/reactNativeSerializer';
...
export function CodeView({ pages, className = '' }: CodeViewProps) {
  ...
  const generateCode = () => {
    return format === 'react' 
      ? serializeToReact(pages)
      : serializeToReactNative(pages);
  };
  ...
  return (
    <div className={`flex flex-col h-full ${className}`}>
      ...
      <div className="px-4 py-2 border-t border-gray-200 bg-gray-50 text-xs text-gray-600">
        {code.split('\n').length} lines • {code.length} characters • {format === 'react' ? 'React JSX' : 'React Native'}
      </div>
    </div>
  );
}