...
function updateContainerRecursive(
  ...
  targetId: string,
  updater: (
    item: ContainerEntity | ComponentEntity
  ) => ContainerEntity | ComponentEntity
): ContainerEntity {
  ...
  return {
    ...
    children: container.children.map((child) => {
      if ("children" in child && Array.isArray(child.children)) {
        ...
        return updateContainerRecursive(
          child as ContainerEntity,
          targetId,
          updater
        );
...
function addToContainerRecursive(
  ...
  return {
    ...
    children: container.children.map((child) => {
      if ("children" in child && Array.isArray(child.children)) {
        return addToContainerRecursive(
          child as ContainerEntity,
          targetId,
          newChild
        );
...
function deleteFromContainerRecursive(
  ...
  return {
    ...
    children: container.children
      ...
      .map((child) => {
        if ("children" in child && Array.isArray(child.children)) {
          return deleteFromContainerRecursive(
            child as ContainerEntity,
            targetId
          );
        }
...
function findInContainerRecursive(
  ...
  for (const child of container.children) {
    ...
    if ("children" in child && Array.isArray(child.children)) {
      const found = findInContainerRecursive(
        child as ContainerEntity,
        targetId
      );
...
export function useBuilderState() {
  ...
  const addComponent = useCallback(
    (containerId: string, type: ComponentType) => {
  ...
  const updatePage = useCallback(
    (pageId: string, updates: Partial<PageEntity>) => {
      setState((prev) => {
        const updatedPages = prev.pages.map((page) =>
  ...
  const updateContainer = useCallback(
    (containerId: string, updates: Partial<ContainerEntity>) => {
      setState((prev) => {
        const updatedPages = prev.pages.map((page) => ({
          ...page,
          children: page.children.map((container) =>
            updateContainerRecursive(container, containerId, (item) => ({
              ...item,
              ...updates,
            }))
          ),
  ...
  const updateComponent = useCallback(
    (componentId: string, updates: Partial<ComponentEntity>) => {
      setState((prev) => {
        const updatedPages = prev.pages.map((page) => ({
          ...page,
          children: page.children.map((container) =>
            updateContainerRecursive(container, componentId, (item) => ({
              ...item,
              ...updates,
            }))
          ),
  ...
  const deleteEntity = useCallback(
    (entityId: string, entityType: "Page" | "Container" | "Component") => {
      setState((prev) => {
        ...
        if (entityType === "Page") {
          ...
          updatedPages = prev.pages.map((page) => ({
            ...
            children: page.children
              ...
              .map((container) =>
                deleteFromContainerRecursive(container, entityId)
              ),
          }));
        }
        const maps = rebuildGlobalMaps(updatedPages);
        return {
          ...
          ...maps,
          selection:
            prev.selection?.entityId === entityId ? null : prev.selection,
        };
      });
    },
    []
  ...
  const updateHierarchyItems = useCallback(
    (items: Map<string, HierarchyViewItem>) => {
      setHierarchyItems(new Map(items));
    },
    []
  );

  const updateHierarchyItem = useCallback(
    (id: string, updates: Partial<HierarchyViewItem>) => {
